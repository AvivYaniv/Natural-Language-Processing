...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    ### YOUR CODE HERE
    random_indices = np.random.choice(num_of_examples, BATCH_SIZE, replace=False)

    for i, index in enumerate(random_indices):
        data[i], labels[i] =                                \
            num_to_word_embedding[in_word_index[index]],    \
            int_to_one_hot(out_word_index[index], output_dim)
...
def eval_neural_lm(eval_data_path):
    ...
    perplexity = 0
    ### YOUR CODE HERE
    for i, index in enumerate(in_word_index):
        data                =   num_to_word_embedding[index]
        perplexity          +=  np.log2(forward(data, out_word_index[i], params, dimensions))
        
    l = perplexity / num_of_examples
