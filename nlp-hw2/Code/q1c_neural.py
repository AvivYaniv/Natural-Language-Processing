...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    cost        = np.sum(-np.multiply(log_y_hat, labels))
    ### END YOUR CODE

    ### YOUR CODE HERE: backward propagation
    gradCost    = y_hat - labels
    
    gradh       = np.dot(gradCost, W2.T)
    
    gradz1      = np.multiply(gradh, sigmoid_grad(h))
    
    gradb1      = np.sum(gradz1,    0).reshape(1, H)
    gradb2      = np.sum(gradCost,  0).reshape(1, Dy)
    
    gradW1      = np.dot(data.T,    gradz1)
    gradW2      = np.dot(h.T,       gradCost)
    ### END YOUR CODE
