...
class Trainer(TrainerBase):
    def __init__(self, model, config, helper, logger):
        ...
        self._loss_function = lambda masked_tag_probs, masked_labels : torch.nn.NLLLoss(ignore_index = 7, reduction = 'sum')(masked_tag_probs, masked_labels) / config.batch_size
        ### YOUR CODE HERE
    ...
    def _batch_loss(self, tag_probs, labels, masks):
        ...
        masked_tag_probs    = tag_probs.transpose(1, 2)
        masked_labels       = labels.masked_fill(~masks, 7).long()
        ### YOUR CODE HERE
...
class DataPreprocessor(BaseDataPreprocessor):
    def pad_sequences(self, examples):
        ...
        for sentence, labels in examples:
            ...
            length_diff     = max_length - sentence_length 
            new_sentence    = sentence                  + [zero_vector] * length_diff
            new_labels      = labels                    + [zero_label]  * length_diff
            masking         = [True] * sentence_length  + [False]       * length_diff            
            ret.append([new_sentence[:max_length], new_labels[:max_length], masking[:max_length]])
            ### YOUR CODE HERE
        return ret
